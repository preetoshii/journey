/*
  ...
  - Within each goal's section, it maps over `detailScreenTypes`.
  - Each `detailScreenType` provides a label and a component.
The `DetailArea` renders the label and then invokes the associated component, passing the current `goal` data to it.
  - The actual rendering of specific details (progress bars, action lists, etc.) is handled by the
    components defined in `detailScreenTypes`.
...
import { nodes as moonNodes } from '../Moon/MoonVisualizer';
